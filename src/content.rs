...
pub struct Content<T: Encodable + Decodable>(
    Vec<u8>,
    PhantomData<T>,
);
...
mod tests {
    ...
    fn encode() {
        ...
        assert_eq!(encoded, Content(vec![0u8, 0, 0, 0, 0, 0, 0, 4, 97, 98, 99, 100], PhantomData));
    }

    #[test]
    fn decode_content_abcd() {
        assert_eq!(Content::<String>(vec![0u8, 0, 0, 0, 0, 0, 0, 4, 97, 98, 99, 100], PhantomData).decode(), "abcd".to_string());
    }
}
